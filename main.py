...
def initialize(screen):
    """
    initialize(screen)
       Эта функция берет изображение и создает наборы тайлов для
        игрока, а затем отображает их на экране
    """
...
def tile_check(tile):
    '''
    tile_check(tile)
        эта функция принимает плитку в качестве кортежа и возвращает список
плитки в ее подходящем состоянии (в виде кортежа), а положение
где он должен воспроизводиться (в виде строки).
Если плитка может воспроизводиться с обеих сторон, возвращаемый список будет
иметь 4 элемента, которые являются обоими состояниями, которые может иметь плитка.
    '''
...
def play(tile, screen, place = None):
    ...
    play(tile, screen, place)
        эта функция принимает плитку в качестве "кортежа", отображает
и место слева или справа, затем рисует плитку на экране.
    """
...
def computer_play(auto_player, screen):
    ...
    computer_play(auto_player, screen)
        эта функция берет объект изображение,
затем выполняет все необходимые действия, связанные с ботом.
    '''
...
def human_play(tile, exact_tile, screen, position, auto_player):
    ...
    human_play(tile, exact_tile, screen, position, auto_player)
        эта функция отвечает за все действия, которые должны быть
предприняты, когда человек нажимает на подходящую плитку.
    '''
...
def switch_pass_button(action, screen):
    ...
    switch_pass_button(action, screen)
        переключает кнопку PASS в положение "включено" или "отключено" в соответствии с
действием, которое вы передаете ему.
    '''
...
def human_has_suitable_tile():
    ...
    human_has_suitable_tile()
        эта функция возвращает значение True, если есть какая-либо
плитка для воспроизведения в списке HUMAN_TILES.
    """
...
def check_if_clicked_on_exit(screen, x, y, check_for_popup = 0):
    ...
    check_if_clicked_on_exit(screen, x, y, check_for_popup = 0)
        эта функция проверяет, нажал ли пользователь на кнопку "выйти".
или кнопки "заново", затем он выполняет необходимые действия
    '''
...
def END_GAME(screen):
    '''
    END_GAME(screen)
     Эта функция вызывается, когда игра заканчивается.
Она определяет, кто выиграл, и показывает определённое сообщение
...
def RESET_GAME():
    '''
    RESET_GAME()
       Сбрасывает все игровые счетчики и фишки, перераспределяет новые фишки
для всех игроков и перерисовывает все на экране.
    '''
